...
| Синхронизация import_clients | `sync_import_clients.py` | вместе с amoCRM 01:10 (2-й шаг) | значения из «Клиенты 2025 / Сводная» → лист `import_clients` (вместо IMPORTRANGE) | месячная таблица `SPREADSHEET_ID`, лист «import_clients» |
...
- **import_clients и IMPORTRANGE (была засада в августе).** Блок оплат в отчёте (Всего будущих
  ...
  теперь `sync_import_clients.py` (сервис-аккаунт) сам пишет туда значения каждую ночь.
